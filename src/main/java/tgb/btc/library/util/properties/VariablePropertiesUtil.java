...
public class VariablePropertiesUtil {
    ...
    public static String getVariable(VariableType variableType) {
        String text;
        try {
            text = IPropertiesPath.VARIABLE_PROPERTIES.getString(variableType.getKey());
        } catch (Exception e) {
    ...
    public static String getVariable(VariableType variableType, DealType dealType, CryptoCurrency cryptoCurrency) {
        ...
        String key = variableType.getKey() + "."
                ...
                + cryptoCurrency.getShortName();
        try {
            text = IPropertiesPath.VARIABLE_PROPERTIES.getString(key);
        } catch (Exception e) {
            throw new BaseException("Переменная по ключу " + key + " не найдена.");
        }
        if (Objects.isNull(text))
    ...
    public static BigDecimal getBigDecimal(String key) {
        return IPropertiesPath.VARIABLE_PROPERTIES.getBigDecimal(key);
    }

    public static Double getDouble(String key) {
        return IPropertiesPath.VARIABLE_PROPERTIES.getDouble(key);
    }

    public static String getWallet(CryptoCurrency cryptoCurrency) {
        return IPropertiesPath.VARIABLE_PROPERTIES.getString(VariableType.WALLET.getKey(cryptoCurrency));
    }
}
